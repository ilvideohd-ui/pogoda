код приложения «Дневник погоды». Он выполняет все ваши требования, но написан так, чтобы его было легко читать и понимать.

Код main.py
Этот код не использует сложные структуры вроде классов, чтобы логика была максимально прозрачной.
...
# --- Запуск приложения ---
root.mainloop()
Как запустить этот простой код
Создайте файлы:
Создайте папку.
В ней создайте файл main.py и вставьте туда код выше.
В той же папке создайте пустой файл weather.json.
Запустите:
Откройте терминал в этой папке.
Введите команду python main.py.
Используйте:
Заполните поля и нажмите «Добавить запись».
Используйте поля фильтра и кнопки «Применить» / «Сбросить».